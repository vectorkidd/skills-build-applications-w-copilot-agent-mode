  ...
  useEffect(() => {
    ...
    fetch(endpoint)
      ...
      .then((payload) => {
        ...
        console.log('[Users] Received data', payload);
        setData(payloadData);
      })
      .catch((fetchError) => {
      ...
      <div className="card-body">
        <form className="row g-2 align-items-center mb-3">
          <div className="col-md-8">
            <label htmlFor="usersFilter" className="form-label visually-hidden">
              Filter users
            </label>
            <input
              id="usersFilter"
          ...
          <div className="col-auto">
            <span className="badge bg-info text-dark">{Array.isArray(data) ? `${data.length} users` : data ? 'Loaded' : 'Pending'}</span>
          </div>
        </form>

        {error && <div className="alert alert-danger">{error}</div>}
        {!data && !error && <div className="text-center text-muted">Loading users...</div>}
            ...
            <table className="table table-hover table-bordered table-sm align-middle mb-0">
              ...
              <tbody>
                {displayedData.map((item) => (
                  <tr key={item.id ?? item.pk ?? item.username ?? Math.random()}>
        ...
        {Array.isArray(displayedData) && displayedData.length === 0 && !error && (
          <div className="alert alert-warning mb-0">No users match your search.</div>
        )}
          ...
          <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Users API Response</h5>
