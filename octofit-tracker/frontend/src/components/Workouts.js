  ...
  useEffect(() => {
    ...
    fetch(endpoint)
      ...
      .then((payload) => {
        ...
        console.log('[Workouts] Received data', payload);
        setData(payloadData);
      })
      .catch((fetchError) => {
      ...
      <div className="card-body">
        <form className="row g-2 align-items-center mb-3">
          ...
          <div className="col-auto">
            <span className="badge bg-info text-dark">{Array.isArray(data) ? `${data.length} workouts` : data ? 'Loaded' : 'Pending'}</span>
          </div>
        </form>

        {error && <div className="alert alert-danger">{error}</div>}
        {!data && !error && <div className="text-center text-muted">Loading workouts...</div>}
        ...
        {Array.isArray(displayedData) && displayedData.length === 0 && !error && (
          <div className="alert alert-warning mb-0">No workouts match your filter criteria.</div>
        )}
          ...
          <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Workouts API Response</h5>
