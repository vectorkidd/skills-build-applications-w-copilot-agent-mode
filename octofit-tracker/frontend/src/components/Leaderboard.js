  ...
  useEffect(() => {
    ...
    fetch(endpoint)
      ...
      .then((payload) => {
        ...
        console.log('[Leaderboard] Received data', payload);
        setData(payloadData);
      })
      .catch((fetchError) => {
      ...
      <div className="card-body">
        <form className="row g-2 align-items-center mb-3">
          ...
          <div className="col-auto">
            <span className="badge bg-info text-dark">{Array.isArray(data) ? `${data.length} entries` : data ? 'Loaded' : 'Pending'}</span>
          </div>
        </form>

        {error && <div className="alert alert-danger">{error}</div>}
        {!data && !error && <div className="text-center text-muted">Loading leaderboard...</div>}
        ...
        {Array.isArray(displayedData) && displayedData.length === 0 && !error && (
          <div className="alert alert-warning mb-0">No leaderboard items match your filter.</div>
        )}
          ...
          <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Leaderboard API Response</h5>
